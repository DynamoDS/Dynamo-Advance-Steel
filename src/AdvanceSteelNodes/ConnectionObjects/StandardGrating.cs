...
namespace AdvanceSteel.Nodes.ConnectionObjects
{
	...
	public class StandardGrating : GraphicObject
	{
		...
		internal StandardGrating(string strClass, string strName, Plane plane, Point3d ptCenter, double dWidth, SteelGeometry.Point3d point1, SteelGeometry.Point3d point2, Vector3d vx, Vector3d vy)
		{
			lock (access_obj)
			{
				using (var ctx = new SteelServices.DocContext())
				{
					...
					else
					{
						...
						if (gratings != null && gratings.IsKindOf(FilerObject.eObjectType.kGrating))
						{
							...
							gratings.SetLength(ObjectsConnection.GetRectangleHeight(point1, point2, vx), true);
		...
		public static StandardGrating BySquare(Autodesk.DesignScript.Geometry.Rectangle rectangle, string gratingClass, string gratingName)
		{
			...
			return new StandardGrating(gratingClass, gratingName, plane, refPoint, Utils.ToInternalUnits(rectangle.Width, true), astCorners[0], astCorners[2], vx, vy);
		}
