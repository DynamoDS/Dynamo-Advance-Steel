...
namespace AdvanceSteel.Nodes.ConnectionObjects.Bolts
{
      ...
      lock (access_obj)
      {
        using (var ctx = new SteelServices.DocContext())
        {
          ...
          if (string.IsNullOrEmpty(handle) || Utils.GetObject(handle) == null)
          {
            ...
            if (boltData.XSpacing > 0) { bolts.Dx = boltData.XSpacing; }
            if (boltData.YSpacing > 0) { bolts.Dy = boltData.YSpacing; }
            if (boltData.XCount > -1) { bolts.Nx = boltData.XCount; }
            if (boltData.YCount > -1) { bolts.Ny = boltData.YCount; }
    ...
    /// <param name="boltData"> Bolt Build Properties </param>
    public static RectangularBoltPattern ByRectangle(Autodesk.DesignScript.Geometry.Rectangle rectangle, 
                                                      ...
                                                      PropertiesBolts boltData)
		{
			...
			return new RectangularBoltPattern(astCorners[0], astCorners[2], handlesList, vx, vy, boltData);
		}
                                                  ...
                                                  PropertiesBolts boltData)
    {
      ...
      return new RectangularBoltPattern(Utils.ToAstPoint(connectionPoint, true), vx, vy, handlesList, boltData);
    }
