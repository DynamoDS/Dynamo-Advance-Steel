...
namespace AdvanceSteel.Nodes.ConnectionObjects
{
	...
	public class RectangularBoltPattern : GraphicObject
	{
		...
		internal void UpdateBoltPattern(ref Autodesk.AdvanceSteel.Modelling.FinitRectScrewBoltPattern toUpdate, int nx, int ny, Point3d point1, Point3d point2)
		{
			toUpdate.Nx = nx;
			toUpdate.Ny = ny;
		...
		internal RectangularBoltPattern(SteelGeometry.Point3d astPoint1, SteelGeometry.Point3d astPoint2, IEnumerable<string> handlesToConnect, SteelGeometry.Vector3d vx, SteelGeometry.Vector3d vy, int nx, int ny)
		{
			lock (access_obj)
			{
				using (var ctx = new SteelServices.DocContext())
				{
					...
					if (string.IsNullOrEmpty(handle) || Utils.GetObject(handle) == null)
					{
						...
						UpdateBoltPattern(ref bolts, nx, ny, astPoint1, astPoint2);
						bolts.WriteToDb();
		...
		public static RectangularBoltPattern ByRectangle(Autodesk.DesignScript.Geometry.Rectangle rectangle, IEnumerable<SteelDbObject> objectsToConnect, int nx, int ny)
		{
			...
			return new RectangularBoltPattern(astCorners[0], astCorners[2], handlesList, vx, vy, nx, ny);
		}
